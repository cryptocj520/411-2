...
class BackpackAPI:
    ...
    async def place_order(
        ...
        retry_delay: float = 0.5  # 添加重试延迟参数
    ) -> Dict:
        """
        下单接口
        ...
        while retries <= max_retries:
            try:
                ...
                for pair in self.config.get("trading_pairs", []):
                    if pair.get("symbol") == symbol.split("_")[0]:  # 提取币种名称
                        trading_pair_config = pair
                        break
                
                # 根据交易对确定数量和价格精度
                if trading_pair_config:
                    quantity_precision = trading_pair_config.get("size_precision", 3)
                    price_precision = trading_pair_config.get("price_precision", 2)
                ...
                if order_type == "LIMIT":
                    ...
                    # 调整价格精度和tick_size
                    price_steps = round(price / tick_size)
                    adjusted_price = price_steps * tick_size
                    formatted_price = "{:.{}f}".format(adjusted_price, price_precision)
                    ...
                    if response.status_code != 200:
                        ...
                        if "Request has expired" in error_text:
                            ...
                            if retries <= max_retries:
                                retry_delay_with_jitter = retry_delay * (1 + 0.2 * random.random())  # 添加随机抖动
                                self.logger.warning(f"请求已过期，将在{retry_delay_with_jitter:.2f}秒后进行第{retries}次重试")
                                await asyncio.sleep(retry_delay_with_jitter)
                                continue
                            else:
                                self.logger.error(f"请求过期，已达到最大重试次数{max_retries}")
                ...
                # 如果是我们抛出的值错误，直接抛出
                if "Backpack下单失败" in str(e):
                    last_error = e
                    # 如果不是请求过期错误，或已达到最大重试次数，则抛出
                    if "Request has expired" not in str(e) or retries >= max_retries:
                        raise
                else:
                    # 其他ValueError直接抛出
                    raise
                ...
                if retries <= max_retries:
                    ...
                    self.logger.warning(f"遇到错误: {e}，将在{retry_delay_with_jitter:.2f}秒后进行第{retries}次重试")
                    await asyncio.sleep(retry_delay_with_jitter)
                else:
                    self.logger.error(f"Backpack下单失败，已达到最大重试次数{max_retries}，最后错误: {e}")
                    raise
        ...
        if last_error:
            ...
            raise ValueError(f"Backpack下单失败，已达到最大重试次数{max_retries}")
