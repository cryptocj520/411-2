...
class ArbitrageEngine:
    ...
    async def _check_open_conditions_without_execution(
        ...
        """
        检查是否满足开仓条件但不执行开仓 - 简化版，只检查预期收益和滑点
        """
        ...
        # 如果深度数据不存在，尝试从交易所API获取 (保留获取深度数据的代码)
        if not bp_depth or not hl_depth:
            try:
                ...
                self.logger.error(f"获取{symbol}深度数据时出错: {e}")
                import traceback
                self.logger.error(f"详细错误信息: {traceback.format_exc()}")
        ...
        # 检查滑点是否超过阈值
        open_conditions = self.config["strategy"]["open_conditions"]
        max_slippage_percent = open_conditions.get("max_slippage_percent", 0.6)
        ignore_high_slippage = open_conditions.get("ignore_high_slippage", False)
        ...
        # 计算预期资金费率收益
        check_expected_profit = open_conditions.get("check_expected_profit", True)
        min_expected_profit = open_conditions.get("min_expected_profit", 0.0005)
        
        # 计算预期资金费率收益（考虑8小时周期）
        expected_funding_profit = 0
        
        # 防御性检查确保资金费率不是None
        if bp_funding is None:
            self.logger.warning(f"{symbol} - BP资金费率为None，使用默认值0")
            bp_funding = 0
            
        if adjusted_hl_funding is None:
            self.logger.warning(f"{symbol} - HL调整后资金费率为None，使用默认值0")
            adjusted_hl_funding = 0
            
        # 记录详细的资金费率信息用于调试
        self.logger.debug(
            f"{symbol} - 详细资金费率: BP={bp_funding:.6f}, HL原始={hl_funding if hl_funding is not None else 0:.6f}, HL调整后={adjusted_hl_funding:.6f}"
        )
        
        if bp_funding < 0 and adjusted_hl_funding < 0:
            # 两个交易所都是负费率
            self.logger.debug(f"{symbol} - 两个交易所都是负费率")
            
            # 比较绝对值大小
            if abs(bp_funding) < abs(adjusted_hl_funding):
                # BP做空(支付少量费用)，HL做多(支付更多费用) - 这种情况通常是亏损的
                expected_funding_profit = abs(bp_funding) - abs(adjusted_hl_funding)
                trade_direction = "BP做空HL做多"
                self.logger.debug(
                    f"{symbol} - BP资金费率绝对值较小，如果{trade_direction}，预期收益: {expected_funding_profit:.6f}"
                )
            else:
                # BP做多(支付更多费用)，HL做空(支付少量费用) - 这种情况也通常是亏损的
                expected_funding_profit = abs(adjusted_hl_funding) - abs(bp_funding)
                trade_direction = "BP做多HL做空"
                self.logger.debug(
                    f"{symbol} - HL资金费率绝对值较小，如果{trade_direction}，预期收益: {expected_funding_profit:.6f}"
                )
                
        elif bp_funding > 0 and adjusted_hl_funding > 0:
            # 两个交易所都是正费率
            self.logger.debug(f"{symbol} - 两个交易所都是正费率")
            
            if bp_funding > adjusted_hl_funding:
                # BP做空(收取更多费用)，HL做多(收取少量费用) - 这种情况通常是盈利的
                expected_funding_profit = bp_funding - adjusted_hl_funding
                trade_direction = "BP做空HL做多"
                self.logger.debug(
                    f"{symbol} - BP资金费率较大，如果{trade_direction}，预期收益: {expected_funding_profit:.6f}"
                )
            else:
                # BP做多(收取少量费用)，HL做空(收取更多费用) - 这种情况也通常是盈利的
                expected_funding_profit = adjusted_hl_funding - bp_funding
                trade_direction = "BP做多HL做空"
                self.logger.debug(
                    f"{symbol} - HL资金费率较大，如果{trade_direction}，预期收益: {expected_funding_profit:.6f}"
                )
                
        else:
            # 一正一负 - 传统资金费率套利
            self.logger.debug(f"{symbol} - 一个交易所正费率，一个交易所负费率")
            
            if bp_funding > 0:
                # BP收取费用，HL支付费用
                expected_funding_profit = bp_funding + abs(adjusted_hl_funding)
                trade_direction = "BP做空HL做多"
                self.logger.debug(
                    f"{symbol} - BP正费率HL负费率，如果{trade_direction}，预期收益: {expected_funding_profit:.6f}"
                )
            else:
                # BP支付费用，HL收取费用
                expected_funding_profit = abs(bp_funding) + adjusted_hl_funding
                trade_direction = "BP做多HL做空"
                self.logger.debug(
                    f"{symbol} - BP负费率HL正费率，如果{trade_direction}，预期收益: {expected_funding_profit:.6f}"
                )
        
        # 检查预期收益是否满足要求
        if check_expected_profit and expected_funding_profit < min_expected_profit:
            message = f"{symbol}预期资金费率收益不足({expected_funding_profit:.6f} < {min_expected_profit})，不建议开仓"
            self.logger.warning(message)
            self._safe_add_condition_message(message)
            return False
        ...
        message = (
            ...
            f"资金费率: BP={bp_funding:.6f}, HL调整后={adjusted_hl_funding:.6f}, "
            f"预期收益: {expected_funding_profit:.6f}, "
            f"滑点: BP={bp_slippage_percent:.4f}%, HL={hl_slippage_percent:.4f}%"
        )

        self._safe_add_condition_message(message)
        self.logger.info(f"{symbol}满足开仓条件，建议开仓")
    ...
    async def _check_close_conditions_without_execution(
        ...
        """
        检查是否满足平仓条件，但不执行平仓 - 简化版，只检查预期收益和滑点
        ...
        """
        # 获取平仓条件配置
        close_conditions = self.config["strategy"]["close_conditions"]
        
        # 读取预期收益检查相关配置
        check_expected_profit = close_conditions.get("check_expected_profit", True)
        min_expected_profit = close_conditions.get("min_expected_profit", 0.0002)

        # 检查持仓时间是否足够长
        min_position_time = close_conditions.get("min_position_time", 30)  # 最短持仓时间
        current_time = time.time()
        ...
        # 获取实际持仓方向
        bp_is_long = bp_entry_side == "BUY"
        hl_is_long = hl_entry_side == "BUY"
        
        # 记录持仓和市场信息
        self.logger.debug(
            f"{symbol} - 平仓检查 - 持仓方向: BP={'多' if bp_is_long else '空'}, HL={'多' if hl_is_long else '空'}, "
            f"资金费率: BP={bp_funding:.6f}, HL调整后={adjusted_hl_funding:.6f}"
        )
        ...
        if bp_funding < 0 and adjusted_hl_funding < 0:
            # 两个交易所都是负费率
            if bp_is_long and not hl_is_long:
                # BP做多(支付费用)，HL做空 - 检查BP支付是否少于HL收取
                expected_funding_profit = abs(adjusted_hl_funding) - abs(bp_funding)
            elif not bp_is_long and hl_is_long:
                # BP做空，HL做多(支付费用) - 检查BP收取是否多于HL支付
                expected_funding_profit = abs(bp_funding) - abs(adjusted_hl_funding)
                
        elif bp_funding > 0 and adjusted_hl_funding > 0:
            # 两个交易所都是正费率
            if not bp_is_long and hl_is_long:
                # BP做空(收取费用)，HL做多 - 检查BP收取是否多于HL收取
                expected_funding_profit = bp_funding - adjusted_hl_funding
            elif bp_is_long and not hl_is_long:
                # BP做多，HL做空(收取费用) - 检查HL收取是否多于BP收取
                expected_funding_profit = adjusted_hl_funding - bp_funding
        ...
        # 记录预期收益信息
        self.logger.info(
            f"{symbol} - 当前持仓方向(BP={bp_entry_side}, HL={hl_entry_side})的预期资金费率收益: {expected_funding_profit:.6f}"
        )
        
        # 根据预期收益决定是否平仓
        funding_condition_met = False
        if check_expected_profit:
            funding_condition_met = expected_funding_profit < min_expected_profit
            if funding_condition_met:
                self.logger.info(
                    f"{symbol} - 预期资金费率收益({expected_funding_profit:.6f})低于阈值({min_expected_profit})，满足平仓条件"
                )

        # 检查滑点条件
        max_close_slippage = close_conditions.get("max_close_slippage_percent", 0.8)
        ignore_close_slippage = close_conditions.get("ignore_close_slippage", False)

        # 获取市场数据中的滑点信息
        market_data = self.data_manager.get_all_data()
        total_slippage = market_data.get(symbol, {}).get("total_slippage", 0.5)  # 默认为0.5%
        ...
        if not slippage_condition_met:
            self.logger.debug(
                f"{symbol} - 预估平仓滑点({total_slippage:.4f}%)超过最大允许值({max_close_slippage:.4f}%)，暂不平仓")
        ...
        position = {
            ...
            "backpack": bp_position,
            "hyperliquid": hl_position
        }
        ...
        if should_close:
            reason = f"预期资金费率收益({expected_funding_profit:.6f})低于阈值({min_expected_profit})，确认平仓"
        
        # 记录条件判断结果
        self.logger.info(
            f"{symbol} - 平仓条件检查: 预期收益条件{'' if funding_condition_met else '未'}满足 "
            f"(当前: {expected_funding_profit:.6f}, 阈值: {min_expected_profit})"
        )
                ...
                try:
                    ...
                    if bp_quantity == 0:
                        ...
                        if single_trade_size and single_trade_size < bp_quantity:
                            self.logger.info(
                                f"{symbol} Backpack采用分批平仓策略，单次交易量: {single_trade_size}")

                            batch_count = math.ceil(
                                bp_quantity / single_trade_size)
                            self.logger.info(
                                f"{symbol} Backpack总计需要{batch_count}批次平仓")

                            for batch in range(1, batch_count + 1):
                                # 计算当前批次的交易量
                                current_batch_size = min(
                                    single_trade_size, remaining_quantity)
                                current_batch_size = round(
                                    current_batch_size, 8)  # 避免浮点数精度问题
                ...
                try:
                    ...
                    if hl_quantity == 0:
                        ...
                        if single_trade_size and single_trade_size < hl_quantity:
                            self.logger.info(
                                f"{symbol} Hyperliquid采用分批平仓策略，单次交易量: {single_trade_size}")

                            batch_count = math.ceil(
                                hl_quantity / single_trade_size)
                            self.logger.info(
                                f"{symbol} Hyperliquid总计需要{batch_count}批次平仓")
    ...
    def _update_position_direction_info(
        ...
        """更新持仓方向信息"""
        self.logger.info("正在更新持仓方向信息...")

        # 获取所有在API查询到的持仓币种
        api_position_symbols = set()
        
        # 从BP持仓中提取币种
        for bp_pos_symbol, bp_position in bp_positions.items():
            symbol = bp_pos_symbol.split("_")[0]  # 从BP符号提取基础币种
            api_position_symbols.add(symbol)
            
        # 从HL持仓中提取币种
        for hl_symbol in hl_positions.keys():
            api_position_symbols.add(hl_symbol)
        ...
        for symbol in api_position_symbols:
            ...
            # 获取持仓方向
            bp_position_side = None
            hl_position_side = None
            
            if bp_symbol in bp_positions:
                bp_position = bp_positions[bp_symbol]
                bp_position_side = bp_position.get("side")
                
            if symbol in hl_positions:
                hl_position = hl_positions[symbol]
                hl_position_side = hl_position.get("side")
                
            # 更新市场数据中的持仓方向
            if symbol in market_data:
                market_data[symbol]["bp_position_side"] = bp_position_side
                market_data[symbol]["hl_position_side"] = hl_position_side

            # 记录持仓方向信息
            self.logger.info(
                f"{symbol} - 持仓方向: BP={bp_position_side or '无'}, HL={hl_position_side or '无'}")
            
            # 获取当前最新的资金费率
            bp_funding = 0
            hl_funding = 0
            adjusted_hl_funding = 0  # 调整后的HL资金费率（乘以8）
            
            if symbol in market_data:
                bp_funding = market_data[symbol].get("bp_funding", 0)
                hl_funding = market_data[symbol].get("hl_funding", 0)
                
                # 调整HL资金费率，乘以8用于对应BP的8小时周期
                adjusted_hl_funding = hl_funding * 8
                
                # 更新市场数据，增加调整后的HL资金费率字段
                market_data[symbol]["adjusted_hl_funding"] = adjusted_hl_funding
                
                # 获取价格数据
                bp_price = market_data[symbol].get("bp_price", 0)
                hl_price = market_data[symbol].get("hl_price", 0)
                
                # 首先计算并保存最优交易方向，无论是否有持仓
                _, _, funding_bp_side, funding_hl_side = self.check_position_profit_status(
                    symbol, bp_price, hl_price, bp_funding, adjusted_hl_funding)
                
                # 将最优交易方向保存到市场数据中
                market_data[symbol]["funding_bp_side"] = funding_bp_side
                market_data[symbol]["funding_hl_side"] = funding_hl_side
                
                # 计算持仓盈亏状态 - 如果有持仓
                if bp_position_side and hl_position_side:
                    # 获取当前的资金费率（而不是使用调用函数时的值）
                    # 注意：这里直接从market_data中获取最新的资金费率
                    current_bp_funding = market_data[symbol].get("backpack", {}).get("funding_rate", 0)
                    if current_bp_funding is None:
                        self.logger.warning(f"{symbol} - 获取BP资金费率失败，使用默认值0")
                        current_bp_funding = 0
                        
                    current_hl_funding_raw = market_data[symbol].get("hyperliquid", {}).get("funding_rate", 0)
                    if current_hl_funding_raw is None:
                        self.logger.warning(f"{symbol} - 获取HL原始资金费率失败，使用默认值0")
                        current_hl_funding_raw = 0
                        
                    current_hl_funding = current_hl_funding_raw * 8  # 调整为8小时周期
                    
                    self.logger.info(f"{symbol} - 获取最新资金费率用于计算盈亏: BP={current_bp_funding}, HL原始={current_hl_funding_raw}, HL调整后={current_hl_funding}")
                    
                    # 调用盈亏状态计算方法，使用最新的资金费率
                    is_profitable, expected_profit, _, _ = self.check_position_profit_status(
                        symbol, bp_price, hl_price, current_bp_funding, current_hl_funding, 
                        bp_position_side, hl_position_side)
                    
                    # 检查资金费率是否都接近0
                    if abs(current_bp_funding) < 0.000001 and abs(current_hl_funding) < 0.000001:
                        # 资金费率接近0，显示为"持平"
                        market_data[symbol]["profit_status"] = "持平"
                    else:
                        # 根据计算结果显示盈亏状态
                        market_data[symbol]["profit_status"] = "盈利" if is_profitable else "亏损"
                    
                    # 更新预期收益
                    market_data[symbol]["expected_profit"] = expected_profit
                    
                    # 记录盈亏状态和最优方向信息
                    profit_status = market_data[symbol]["profit_status"]
                    self.logger.info(
                        f"{symbol} - 持仓盈亏状态: {profit_status}, "
                        f"预期收益: {expected_profit:.6f}, 最优方向: BP={funding_bp_side}, HL={funding_hl_side}")

            # 更新开仓时间
            if symbol in self.position_open_times:
                open_time = self.position_open_times[symbol]
                market_data[symbol]["open_time"] = open_time
                
                # 计算持仓时长
                current_time = time.time()
                position_duration = current_time - open_time
                hours, remainder = divmod(position_duration, 3600)
                minutes, seconds = divmod(remainder, 60)
                
                duration_str = f"{int(hours)}小时{int(minutes)}分{int(seconds)}秒"
                market_data[symbol]["position_duration"] = duration_str
                
                self.logger.info(
                    f"{symbol}持仓时间: {duration_str} (开仓于{datetime.fromtimestamp(open_time).strftime('%Y-%m-%d %H:%M:%S')})")
    ...
    def check_position_profit_status(
            self,
            symbol,
            bp_price,
            hl_price,
            ...
            hl_position_side=None):
        """
        计算已持有仓位的盈亏状态，根据当前费率情况和开仓方向判断

        Args:
            symbol: 基础币种，如 "BTC"
            bp_price: Backpack价格
            hl_price: Hyperliquid价格
            bp_funding: Backpack资金费率（8小时周期）
            hl_funding: Hyperliquid资金费率（已调整为8小时周期）
            bp_position_side: Backpack当前持仓方向 ("BUY"或"SELL")
            hl_position_side: Hyperliquid当前持仓方向 ("BUY"或"SELL")

        Returns:
            tuple: (is_profitable, expected_profit, funding_bp_side, funding_hl_side) 是否盈利，预期收益，以及当前资金费率下的最优交易方向
        """
        # 默认为无盈亏状态
        is_profitable = False
        expected_profit = 0.0
        ...
        if bp_funding < 0 and hl_funding < 0:
            # 两个交易所都是负费率
            if abs(bp_funding) > abs(hl_funding):
                # BP绝对值大，最优方向：BP做多，HL做空
                funding_bp_side = "BUY"
                funding_hl_side = "SELL"
            else:
                # HL绝对值大，最优方向：BP做空，HL做多
                funding_bp_side = "SELL"
                funding_hl_side = "BUY"
        elif bp_funding > 0 and hl_funding > 0:
            # 两个交易所都是正费率
            if bp_funding > hl_funding:
                # BP费率大，最优方向：BP做空，HL做多
                funding_bp_side = "SELL"
                funding_hl_side = "BUY"
            else:
                # HL费率大，最优方向：BP做多，HL做空
                funding_bp_side = "BUY"
                funding_hl_side = "SELL"
        else:
            # 一正一负 - 传统资金费率套利
        ...
        # 检查持仓方向是否存在，并计算实际的预期收益
        if bp_position_side and hl_position_side:
            # 根据实际持仓方向计算资金费率收益
            # BP是做多（BUY），则收取负费率，支付正费率
            # BP是做空（SELL），则收取正费率，支付负费率
            bp_profit = 0
            if bp_position_side == "BUY":
                # BP做多，收取负费率，支付正费率
                bp_profit = -bp_funding  # 反向计算（负值变正值，正值变负值）
            else:  # SELL
                # BP做空，收取正费率，支付负费率
                bp_profit = bp_funding

            # HL是做多（BUY），则收取负费率，支付正费率
            # HL是做空（SELL），则收取正费率，支付负费率
            hl_profit = 0
            if hl_position_side == "BUY":
                # HL做多，收取负费率，支付正费率
                hl_profit = -hl_funding  # 反向计算
            else:  # SELL
                # HL做空，收取正费率，支付负费率
                hl_profit = hl_funding

            # 总收益 = BP收益 + HL收益
            expected_profit = bp_profit + hl_profit
            ...
            self.logger.info(
                f"{symbol} 资金费率盈亏计算: "
                f"BP方向={bp_position_side}, BP费率={bp_funding:.6f}, BP收益={bp_profit:.6f}, "
                f"HL方向={hl_position_side}, HL费率={hl_funding:.6f}, HL收益={hl_profit:.6f}, "
                f"总收益={expected_profit:.6f}, 状态={'盈利' if is_profitable else '亏损'}")
        
        return is_profitable, expected_profit, funding_bp_side, funding_hl_side
    ...
    def _sync_records_with_api_positions(self, api_positions):
        ...
        expired_time_records = set(
            self.position_open_times.keys()) - api_positions
        ...
        expired_funding_records = set(
            self.entry_funding_rates.keys()) - api_positions
